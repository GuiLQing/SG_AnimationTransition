...
#import "SG_AnimationInteractiveTransition.h"
#import "SG_AnimationTransitionConfig.h"
#import "SG_AnimationMagicMoveEffect.h"
#import "SG_AnimationSmoothEffect.h"
#import "SG_AnimationTikTokEffect.h"
#import "SG_AnimationMaskEffect.h"

@interface SG_AnimationInteractiveTransition () <UIGestureRecognizerDelegate>
...
@implementation SG_AnimationInteractiveTransition{
    id <UIViewControllerAnimatedTransitioning> _animtedTransitioning;
}

+ (instancetype)shareInteractiveTransition{
    
    static SG_AnimationInteractiveTransition * _shareInteractiveTransition = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _shareInteractiveTransition = [SG_AnimationInteractiveTransition new];
    });
...
- (void)handleAnimatedTransitioning{
    if (self.type == KAnimationTransitionTypeNone){
        ...
        _animtedTransitioning = [[SG_AnimationSmoothEffect alloc] initWithIsBack:self.isBack];
    }else if(self.type == KAnimationTransitionTypeMagicMove){
        _animtedTransitioning = [[SG_AnimationMagicMoveEffect alloc] initWithIsBack:self.isBack];
    }else if(self.type == KAnimationTransitionTypeTikTokComment){
        _animtedTransitioning = [[SG_AnimationTikTokEffect alloc] initWithIsBack:self.isBack];
    }else if(self.type == KAnimationTransitionTypeMask){
        _animtedTransitioning = [[SG_AnimationMaskEffect alloc] initWithIsBack:self.isBack];
    }
}
